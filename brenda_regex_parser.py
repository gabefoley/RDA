...
class AnnotatedProtein():
    ...
    def __init__(self, id, name):
        ...
        self.prot_annots["species"] = " ".join(self.name.split(" ")[0:2])
...
def parse_brenda(filename, annot_dict = {}, uniprot_mapping=False, add_commonalities=True, add_comments=True):
    ...
    # Regex for extracting the primary annotation when it is an annotation common to all (no preceeding list of nums)
    # common_regex = re.compile('((?<=\w{2}[\t]))(?P<primary_annot>.*(?=[\s\t]+[\(]))\s+(?P<comments>([\s\S]*?>\)))')
...
num_to_prot = parse_brenda('brenda_regex.txt', annot_dict, add_commonalities=False, add_comments=False)
